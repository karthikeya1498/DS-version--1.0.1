...
def evaluate_agent(
    agent: PPOAgent, scenario_seeds: list[int], horizon: int
    ...
    for scenario_seed in scenario_seeds:
        ...
        observation = env.reset(scenario_seed)
        done = False
        ppo_total = 0.0
        while not done:
...
def main() -> None:
    ...
    output = root / "data/processed/phase4/ppo_paired_multiseed"
    ...
    for train_seed in train_seeds:
        ...
        agent = PPOAgent(
            ...
            env.action_count,
            PPOConfig(learning_rate=0.005, seed=train_seed),
        )
        agent.train(env, episodes=60)
        ppo, baseline = evaluate_agent(agent, scenario_seeds, horizon)
        rows.append(
            {
                ...
                "ppo_mean_return": float(np.mean(ppo)),
                "baseline_mean_return": float(np.mean(baseline)),
                "delta_mean_return": float(np.mean(np.asarray(ppo) - np.asarray(baseline))),
                "delta_std_return": float(np.std(np.asarray(ppo) - np.asarray(baseline), ddof=1)),
            }
        )
    deltas = np.asarray([row["delta_mean_return"] for row in rows])
    summary = {
        "configuration": {
            "learning_rate": 0.005,
            "horizon": horizon,
            "train_episodes": 60,
            "evaluation_scenarios": len(scenario_seeds),
        },
        "train_seeds": train_seeds,
        "scenario_seeds": scenario_seeds,
        "runs": rows,
        "mean_delta_across_policies": float(deltas.mean()),
        "std_delta_across_policies": float(deltas.std(ddof=1)),
        "policy_wins": int((deltas > 0).sum()),
        "policy_win_rate": float((deltas > 0).mean()),
    }
        ...
        json.dumps(
            {
                key: summary[key]
                for key in (
                    "mean_delta_across_policies",
                    "std_delta_across_policies",
                    "policy_wins",
                    "policy_win_rate",
                )
            },
