...
def make_orders() -> list[Order]:
    ...
    return [
        Order(
            f"o{node}",
            base,
            Location(str(node), "zone", float(node), float(node)),
            node % 3 + 1,
            now,
            TimeWindow(now, now + timedelta(hours=4)),
            priority=4 - node % 3,
        )
...
def route_cost(sequence: list[str]) -> float:
    return sum(
        abs(int(right) - int(left)) * (1 + min(int(left), int(right)) * 0.05)
...
def main() -> None:
    ...
    for method in ["greedy", "greedy_2opt", "greedy_3opt", "simulated_annealing", "genetic"]:
        ...
        result, cost = solver.optimize_sequence(sequence, route_cost, method, seed=42)
        rows.append(
            {
                "algorithm": method,
                "objective": cost,
    ...
    exact_candidates = [
        (["0", *middle, "0"], route_cost(["0", *middle, "0"]))
        for middle in itertools.permutations(["1", "2", "3"])
    ]
    exact_optimum = min(exact_candidates, key=lambda item: item[1])[1]
    heuristic_route, heuristic_cost = solver.optimize_sequence(
        ["0", "1", "2", "3", "0"], route_cost, "genetic", seed=42
    )
    exact_validation = {
        "instance_nodes": 3,
        "exact_optimum": exact_optimum,
        "heuristic_cost": heuristic_cost,
        "matches_exact": abs(exact_optimum - heuristic_cost) < 1e-9,
        "heuristic_route": heuristic_route,
    }
    scalability = []
    for size in [10, 25, 50, 100, 250]:
        candidate = ["0", *[str(index) for index in range(1, size)], "0"]
        started = perf_counter()
        _, value = solver.optimize_sequence(candidate, route_cost, "greedy_2opt", seed=42)
        scalability.append(
            {
                "orders": size,
    ...
    actual = make_orders()
    sensitivity = []
    for label, error, uncertainty in [
        ("actual", 0.0, 0.0),
        ("naive", 0.10, 0.0),
        ("xgboost", 0.03, 0.02),
        ("noisy_5pct", 0.05, 0.05),
        ("noisy_10pct", 0.10, 0.10),
        ("noisy_20pct", 0.20, 0.20),
    ]:
        vehicles = [
            Vehicle(
                "v1",
        ...
        predictions = {
            item.order_id: Prediction(
                item.demand_units * (1 + error), late_risk=error, uncertainty=uncertainty
            )
            for item in actual
        }
        ...
        sensitivity.append(
            {
                ...
                "decision_cost": result.total_cost,
    ...
    (output / "comparison.json").write_text(
        json.dumps(
            {
                "scenario": "fixed_line_graph_seed_42",
                "algorithms": rows,
    ...
    with (output / "comparison.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle, fieldnames=["algorithm", "objective", "distance_km", "runtime_ms", "feasible"]
        )
        writer.writeheader()
        ...
        json.dumps(
            {
                ...
                "exact_validation": exact_validation,
                "scalability_rows": len(scalability),
                "output": str(output),
            },
            indent=2,
        )
