...
def main() -> dict:
    ...
    destination = Location("b", "zone-b", 0, 1)
    graph = RoadGraph()
    graph.add_node(Node("a", 0, 0))
    graph.add_node(Node("b", 0, 1))
    graph.add_edge(Edge("a", "b", 1.0))
    vehicle = Vehicle("v1", origin, 10, now, now + timedelta(hours=2))
    order = Order(
        "o1", origin, destination, round(forecast), now, TimeWindow(now, now + timedelta(hours=1))
    )
    ...
    token = client.post(
        "/api/v1/auth/token",
        json={"username": "system-test", "password": "test", "tenant_id": "tenant-test"},
    ).json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    sim = client.post(
        "/api/v1/simulation/run",
        headers=headers,
        json={"seed": 42, "duration_hours": 1, "zones": 3, "vehicles": 2, "orders_per_hour": 2},
    )
    demand = client.post(
        "/api/v1/forecast/demand", headers=headers, json={"values": [4, 6, 8], "horizon": 1}
    )
    strategies = client.get("/api/v1/routing/strategies", headers=headers)
    assert (
        health.status_code == sim.status_code == demand.status_code == strategies.status_code == 200
    )
    return {
        "forecast_value": forecast,
        "route_cost": route.travel_cost if route else None,
        "vehicle_rows": len(vehicle_dispatch_rows(snapshot)),
        "api_statuses": [
            health.status_code,
            sim.status_code,
            demand.status_code,
