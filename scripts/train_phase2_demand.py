...
def train(
    input_path: str | Path = "data/raw/mobility/hour.csv",
    output_path: str | Path = "data/processed/phase2_demand_metrics.json",
) -> dict:
    frame = pd.read_csv(input_path)
    frame["timestamp"] = pd.to_datetime(frame["dteday"]) + pd.to_timedelta(frame["hr"], unit="h")
    frame["demand"] = frame["cnt"]
    frame["zone"] = "capital_bikeshare"
    features = build_demand_features(frame[["timestamp", "zone", "demand"]], group_col="zone")
