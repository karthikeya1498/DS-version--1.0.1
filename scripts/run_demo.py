...
def main() -> int:
    ...
    print(
        f"TrafficSpeedRangeQuery (Segment Tree) peak window [3-5] min speed: {min_speed_peak} km/h"
    )
    ...
    sim_config = SimulationConfig(
        seed=42, duration=timedelta(hours=2), zones=3, vehicles=4, orders_per_hour=6
    )
    sim = LogisticsSimulator(sim_config)
    sim_result = sim.run()
    print(
        f"Simulation executed: {sim_result.metrics.total_orders} total orders, "
    ...
    eta_row = build_eta_feature_row(
        distance_km=12.5, free_flow_speed_kmh=50.0, traffic_multiplier=1.4
    )
    print(
        f"Extracted ETA feature row: dist={eta_row['distance_km']}, traffic={eta_row['traffic_multiplier']}"
    )
    ...
    print(
        f"Neural MLP forward pass complete. Layer activation dimensions: {[len(a) for a in acts]}"
    )
    ...
    print(
        f"Registered model 'xgb_demand_v1' with dataset_hash: {meta['dataset_hash']}, status: {meta['approval_status']}"
    )
    ...
    sample_orders = [
        Order(
            f"ORD_{i:02d}",
            depot_loc,
            Location("zone_A", "zone_A", 12.98, 77.60),
            demand_units=3,
            created_at=now,
            time_window=TimeWindow(now, now + timedelta(hours=3)),
        )
        for i in range(12)
    ]
    sample_vehicles = [
    ...
    print(
        f"Assigned {len(sample_orders) - len(unassigned)} orders across {len(bundles)} vehicles (Unassigned: {len(unassigned)})"
    )
    for v_id, b_ords in bundles.items():
        print(
            f"  - Vehicle {v_id}: {len(b_ords)} orders, Total Load: {sum(o.demand_units for o in b_ords)} units"
        )
    ...
    stress_orders = [
        Order(
            f"STRESS_{i:02d}",
            depot_loc,
            Location(f"zone_{chr(65 + (i % 4))}", "z", 12.98, 77.60),
            demand_units=2,
            created_at=now,
    ...
    print(
        f"50-Order Stress Result: Served={stress_result.served_orders}, Unserved={stress_result.unserved_orders}, Total Cost=${stress_result.total_cost:.2f}"
    )
    ...
    query_resp = assistant.from_text(
        "Simulate what happens if 25% demand increase and 15% traffic increase"
    )
    ...
    print(
        f"Persisted and retrieved Order record from SQLite: ID={saved.order_id}, Demand={saved.demand_units} units"
    )
