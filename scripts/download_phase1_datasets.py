...
SOURCES = {
    "osm_manhattan": {
        "url": "https://overpass-api.de/api/interpreter?data=[out:json];way[highway](40.70,-74.02,40.80,-73.93);out%20geom;",
        "path": "data/raw/osm/manhattan_roads.json",
        "kind": "osm",
    },
    "nyc_tlc_yellow_2024_01": {
        "url": "https://d37ci6vzurychx.cloudfront.net/trip-data/yellow_tripdata_2024-01.parquet",
        "path": "data/raw/traffic/yellow_tripdata_2024-01.parquet",
        "kind": "traffic",
    },
    "noaa_ghcn_central_park": {
        "url": "https://www.ncei.noaa.gov/pub/data/ghcn/daily/by_station/USW00094728.csv.gz",
        "path": "data/raw/weather/USW00094728.csv.gz",
        "kind": "weather",
    },
    "uci_bike_sharing": {
        "url": "https://archive.ics.uci.edu/static/public/275/bike+sharing+dataset.zip",
        "path": "data/raw/demand/bike_sharing.zip",
        "kind": "demand",
    },
    "uci_logistics_orders": {
        "url": "https://archive.ics.uci.edu/static/public/409/daily+demand+forecasting+orders.zip",
        "path": "data/raw/orders/daily_demand.zip",
        "kind": "orders",
    },
}
...
def write_holidays(year: int = 2024) -> dict:
        ...
        entries = [
            {
                "date": day.isoformat(),
                "holiday": label,
                "event": "federal_holiday",
                "expected_demand_multiplier": 0.8,
            }
            for day, label in sorted(holidays.US(years=year).items())
        ]
    except ImportError:
        entries = [
            {
    ...
    manifest = {
        "name": "us_holidays_2024",
        "kind": "events",
        "local_path": str(target.relative_to(ROOT)),
        "rows": len(entries),
        "source": "python-holidays rules for United States federal holidays",
    }
...
def main(names: list[str] | None = None, force: bool = False) -> list[dict]:
    ...
    (ROOT / "data/raw/dataset_manifest.json").write_text(
        json.dumps(
            {"generated_at": datetime.now(UTC).date().isoformat(), "datasets": manifests}, indent=2
        ),
        encoding="utf-8",
    )
    return manifests
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            main(None if not args.dataset or "all" in args.dataset else args.dataset, args.force),
            indent=2,
        )
    )
