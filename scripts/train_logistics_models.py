...
def evaluate(path='data/raw/logistics/Daily_Demand_Forecasting_Orders.csv', output='data/processed/logistics_forecast_metrics.json'):
    frame = load_data(path)
    features = [column for column in frame.columns if column != 'target']
    split = int(len(frame) * .8)
    train, test = frame.iloc[:split], frame.iloc[split:]
    models = {'gradient_boosting': GradientBoostingRegressor(random_state=42, n_estimators=100, max_depth=2, learning_rate=.05)}
    if XGBRegressor is not None:
        models['xgboost'] = XGBRegressor(n_estimators=150, max_depth=3, learning_rate=.05, subsample=.9, colsample_bytree=.9, objective='reg:squarederror', random_state=42, n_jobs=2)
    results = {'dataset': 'UCI Daily Demand Forecasting Orders', 'rows_after_history': len(frame), 'features': features, 'split': {'train': len(train), 'test': len(test)}, 'models': {}}
    baseline = [float(train.target.mean())] * len(test)
    results['models']['train_mean'] = {'mae': mean_absolute_error(test.target, baseline), 'rmse': mean_squared_error(test.target, baseline) ** .5, 'r2': r2_score(test.target, baseline)}
    for name, model in models.items():
        model.fit(train[features], train.target)
        prediction = model.predict(test[features])
        results['models'][name] = {'mae': mean_absolute_error(test.target, prediction), 'rmse': mean_squared_error(test.target, prediction) ** .5, 'r2': r2_score(test.target, prediction)}
