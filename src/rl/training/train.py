...
def train(episodes: int = 100, seed: int = 42, output: str | Path = 'data/processed/phase4/ppo_training.json') -> dict:
    ...
    result = {'algorithm': 'ppo_linear_reference', 'seed': seed, 'episodes': episodes, 'mean_return': metrics.mean_return, 'final_return': metrics.final_return, 'returns': metrics.returns}
