...
def evaluate(episodes: int = 20, seed: int = 42, output: str | Path = 'data/processed/phase4/evaluation.json') -> dict:
    ...
    for episode in range(episodes):
        observation = env.reset(seed + 1000 + episode); done = False; ppo_total = 0.0; baseline_total = 0.0
        while not done:
        ...
        observation = env.reset(seed + 1000 + episode); done = False
        while not done:
    ...
    result = {'algorithm': 'ppo_linear_reference', 'baseline': 'all_defer', 'seed': seed, 'episodes': episodes, 'ppo_mean_return': float(np.mean(ppo_returns)), 'baseline_mean_return': float(np.mean(baseline_returns)), 'unseen_scenario_seeds': list(range(seed + 1000, seed + 1000 + episodes))}
