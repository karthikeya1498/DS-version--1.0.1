...
class PPOAgent:
    ...
    def act(self, observation: np.ndarray, deterministic: bool = False) -> tuple[int, float, float]:
        ...
        action = (
            int(np.argmax(probs))
            if deterministic
            else int(self.rng.choice(self.action_count, p=probs))
        )
        return (
            action,
            float(np.log(max(probs[action], 1e-12))),
            float(self._features(observation) @ self.value),
        )
    ...
    def update(
        ...
        for _ in range(self.config.epochs):
            for observation, action, old_log_prob, advantage, target in zip(
                ...
                gradient = (
                    (np.eye(self.action_count)[action] - probs)
                    * min(ratio, 1 + self.config.clip_ratio)
                    * advantage
                )
                self.policy += self.config.learning_rate * np.clip(
                    np.outer(features, gradient), -1.0, 1.0
                )
    ...
    def train(self, env, episodes: int = 100) -> TrainingMetrics:
        ...
        return TrainingMetrics(
            ...
            float(returns[-1]) if returns else 0.0,
            tuple(policy_losses),
            tuple(value_losses),
        )
