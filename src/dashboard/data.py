...
def load_osm_edges(root: str | Path, limit: int = 2500) -> pd.DataFrame:
    ...
    if not path.exists(): return pd.DataFrame(columns=['x0', 'y0', 'x1', 'y1'])
    payload = json.loads(path.read_text(encoding='utf-8'))
    rows = []
    for item in payload.get('elements', []):
        geometry = item.get('geometry', []) if item.get('type') == 'way' else []
        for start, end in itertools.pairwise(geometry):
            if None not in (start.get('lon'), start.get('lat'), end.get('lon'), end.get('lat')): rows.append({'x0': start['lon'], 'y0': start['lat'], 'x1': end['lon'], 'y1': end['lat']})
            if len(rows) >= limit: return pd.DataFrame(rows)
...
def load_forecast_metrics(root: str | Path) -> pd.DataFrame:
    ...
    if not path.exists(): path = Path(root) / 'data/processed/forecast_metrics_lagged.json'
    payload = load_json(path, {})
    models = payload.get('models')
    if models: return pd.DataFrame(models).T.reset_index(names='model')
