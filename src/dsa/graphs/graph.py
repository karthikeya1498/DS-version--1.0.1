...
class RoadGraph:
    ...
    def add_edge(self, edge: Edge, bidirectional: bool = False) -> None:
        if edge.source not in self.nodes or edge.target not in self.nodes: raise KeyError('both edge endpoints must exist as nodes')
        self.adjacency[edge.source].append(edge)
        if bidirectional: self.adjacency[edge.target].append(Edge(edge.target, edge.source, edge.weight))
