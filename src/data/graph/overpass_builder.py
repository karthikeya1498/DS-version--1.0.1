...
def build_from_overpass_json(path: str | Path) -> RoadGraph:
    ...
    for element in payload.get("elements", []):
        ...
        for point in geometry:
            node_id = str(point.get("lat")) + ":" + str(point.get("lon"))
            seen[node_id] = (float(point["lat"]), float(point["lon"]))
    for node_id, (latitude, longitude) in seen.items():
        graph.add_node(Node(node_id, latitude, longitude))
    for element in payload.get("elements", []):
        raw_tags = element.get("tags", {})
        tags = (
            raw_tags
            if isinstance(raw_tags, dict)
            else {tag.get("k"): tag.get("v") for tag in raw_tags}
        )
        geometry = element.get("geometry", [])
        ...
        for source, target in pairwise(refs):
            ...
            distance = ((a.latitude - b.latitude) ** 2 + (a.longitude - b.longitude) ** 2) ** 0.5
            graph.add_edge(
                Edge(source, target, max(distance, 1e-9)), bidirectional=tags.get("oneway") != "yes"
            )
    return graph
