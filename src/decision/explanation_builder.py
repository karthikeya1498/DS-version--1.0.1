...
def build_evidence(record: DecisionRecord) -> EvidenceBundle:
    ...
    if selected:
        evidence.append(f"selected_objective={selected.objective}")
    alternatives = tuple(
        f"{candidate.action}: {'feasible' if candidate.feasible else 'rejected'}"
    ...
    uncertainty = tuple(
        f"{key}={value}"
        for key, value in (
            record.prediction_bundle.uncertainty.items() if record.prediction_bundle else []
        )
    )
    return EvidenceBundle(
        record.decision_id, record.selected_action, tuple(evidence), alternatives, uncertainty
    )
