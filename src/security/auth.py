...
def create_token(subject: str, tenant_id: str, scopes: list[str] | None = None, expires_in: int = 3600) -> str:
    header = {"alg": "HS256", "typ": "JWT"}
    payload = {"sub": subject, "tenant_id": tenant_id, "scopes": scopes or ["routing:read", "forecast:read"], "exp": int(time.time()) + expires_in}
...
def decode_token(token: str) -> dict:
    try:
        ...
        if payload.get('exp', 0) < time.time(): raise ValueError("expired token")
        if not payload.get('sub') or not payload.get('tenant_id'): raise ValueError("missing claims")
        return payload
...
def get_current_principal(credentials: HTTPAuthorizationCredentials | None = Depends(_bearer)) -> TenantPrincipal:
