...
def test_login_and_traffic_update_are_tenant_aware(monkeypatch):
    ...
    token = client.post('/api/v1/auth/token', json={'username':'alice','password':'test','tenant_id':'tenant-a'}).json()['access_token']
    response = client.post('/api/v1/traffic/updates', headers={'Authorization': f'Bearer {token}'}, json={'zone_id':'zone-1','multiplier':1.4,'affected_vehicle_ids':['v-1']})
...
def test_websocket_requires_jwt(monkeypatch):
    ...
    client = TestClient(app)
    with pytest.raises(Exception):
        with client.websocket_connect('/api/v1/ws/traffic'):
            pass
    token = create_token('alice', 'tenant-a')
