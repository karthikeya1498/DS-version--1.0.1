...
def test_candidate_ranker_prefers_lowest_feasible_objective():
    candidates = [
        CandidateEvidence("b", "b", True, 2.0),
        CandidateEvidence("a", "a", False, 0.1, "capacity"),
    ]
...
def test_counterfactual_isolated_from_baseline():
    ...
    modified = apply_modification(
        baseline, ScenarioModification(demand_multiplier=1.2, traffic_multiplier=1.3)
    )
...
def test_explanation_is_grounded_in_record():
    record = DecisionRecord(
        ...
        {"cost": 4.0},
        (
            CandidateEvidence("a", "serve", True, 4.0),
            CandidateEvidence("b", "defer", False, 9.0, "late"),
        ),
    )
...
def test_guardrails_reject_mutation_and_unknown_tools():
    ...
    try:
        validate_tool_request(
            ToolRequest(tool="simulate_scenario", arguments={"mutate_state": True})
        )
        assert False
    except ValueError:
        pass
...
def test_assistant_parses_isolated_scenario_request():
    ...
    response = client.post(
        ...
        json={"text": "Simulate a 25% demand spike and 30% traffic increase."},
    )
...
def test_phase5_api_record_explain_and_scenario():
    ...
    response = client.post(
        ...
        json={
            "decision_id": "api-d1",
            "scenario_id": "api-s1",
            "selected_action": "serve",
            "objective_metrics": {"cost": 3.0},
        },
    )
    assert response.status_code == 200
    assert client.get("/api/v1/decisions/api-d1").status_code == 200
    assert (
        client.post("/api/v1/decisions/explain", json={"decision_id": "api-d1"}).json()["grounded"]
        is True
    )
    assert (
        client.post("/api/v1/decisions/compare", json={"decision_id": "api-d1"}).status_code == 200
    )
