...
def graph():
    g = RoadGraph()
    for i in range(5): g.add_node(Node(str(i), latitude=0, longitude=i))
    g.add_edge(Edge('0','1',2), bidirectional=True); g.add_edge(Edge('1','2',2), bidirectional=True)
    g.add_edge(Edge('0','3',1), bidirectional=True); g.add_edge(Edge('3','2',1), bidirectional=True)
...
def test_priority_queue_is_stable_for_equal_priorities():
    q = PriorityQueue(); q.push(1, 'first'); q.push(1, 'second')
    assert q.pop() == (1, 'first'); assert q.pop() == (1, 'second')

def test_negative_edges_are_rejected():
    with pytest.raises(ValueError): Edge('a', 'b', -1)
