...
def make_graph():
    graph = RoadGraph()
    for node_id, longitude in [('a', 0), ('b', 1), ('c', 2)]: graph.add_node(Node(node_id, 0, longitude))
    graph.add_edge(Edge('a','b',1), bidirectional=True); graph.add_edge(Edge('b','c',1), bidirectional=True); graph.add_edge(Edge('a','c',3), bidirectional=True)
    return graph

def test_traffic_updates_only_dynamic_weights_and_reset_restores_base():
    graph = make_graph(); dynamic = TrafficAwareGraph(graph)
    traffic = TrafficState(datetime(2026,1,1,tzinfo=timezone.utc), {'zone-c': 4.0})
...
def test_router_selects_lowest_travel_cost_vehicle():
    graph = make_graph(); now = datetime(2026,1,1,tzinfo=timezone.utc)
    destination = Location('c','zone-c',0,2); origin = Location('a','zone-a',0,0)
    order = Order('o1', origin, destination, 1, now, TimeWindow(now, now.replace(hour=2)))
    vehicles = [Vehicle('far', Location('b','zone-b',0,1), 5, now, now.replace(hour=2)), Vehicle('near', origin, 5, now, now.replace(hour=2))]
