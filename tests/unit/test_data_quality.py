...
def test_quality_engine_accepts_valid_records():
    ...
    result = validate_operational_records(
        [
            {
                "order_id": "o1",
                "distance_km": 2.0,
...
def test_quality_engine_reports_duplicate_and_invalid_values():
    result = validate_operational_records(
        [
            {"order_id": "o1", "distance_km": -1, "latitude": 100},
            {"order_id": "o1", "actual_eta": -3},
        ]
    )
...
def test_quality_engine_reports_temporal_error():
    now = datetime.now(UTC)
    result = validate_operational_records(
        [{"order_id": "o1", "created_at": now, "delivered_at": now - timedelta(seconds=1)}]
    )
    assert result["valid"] is False
