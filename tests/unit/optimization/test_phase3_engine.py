...
def graph() -> RoadGraph:
    result = RoadGraph()
    for node in range(4): result.add_node(Node(str(node), latitude=float(node), longitude=float(node)))
...
def order(order_id='o1', node='2', demand=2):
    ...
    base = Location('0', 'z0')
    return Order(order_id, base, loc, demand, now, TimeWindow(now, now + timedelta(hours=2)), priority=2)
...
def test_objective_weights_and_capacity_dp():
    objective = Objective(ObjectiveWeights(distance=2, fuel=0, lateness=10, unserved=100, vehicle_usage=5))
    assert objective.score(distance_km=3, lateness_minutes=2, unserved_orders=1, activated_vehicles=1) == 131
...
def test_constraint_engine_rejects_capacity_and_unknown_order():
    ...
    route_report = ConstraintEngine().check_route(RoutePlan('v1', ('missing',), ('0',), 0), [order()], [vehicle()])
...
def test_phase3_greedy_connects_prediction_to_graph_route():
    ...
    result = solver.solve([order()], [vehicle()], {'o1': Prediction(demand=2, eta_minutes=3, late_risk=.1)}, method='greedy')
...
def test_genetic_operator_preserves_route_permutation():
    solver = Phase3Solver(GraphDispatchRouter(graph()))
    route, value = solver.optimize_sequence(['0', '3', '1', '2', '0'], lambda path: sum(abs(int(path[i + 1]) - int(path[i])) for i in range(len(path) - 1)), method='genetic')
...
def test_simulated_annealing_never_returns_worse_best_solution():
    ...
    initial = ['0', '1', '2', '3', '0']
    result, cost = optimizer.optimize(initial, lambda route: sum(abs(int(route[i + 1]) - int(route[i])) for i in range(len(route) - 1)))
    assert cost <= 6
