...
def build_grid(side: int, seed: int = 42) -> RoadGraph:
    ...
    for row in range(side):
        for col in range(side):
            graph.add_node(Node(f"{row},{col}", row / side, col / side))
    for row in range(side):
        for col in range(side):
            for dr, dc in ((1, 0), (0, 1)):
                ...
                if nr < side and nc < side:
                    graph.add_edge(
                        Edge(f"{row},{col}", f"{nr},{nc}", 1.0 + rng.random() * 0.25),
                        bidirectional=True,
                    )
    return graph
...
def benchmark(side: int, repetitions: int = 3) -> list[dict]:
    ...
    start, goal = "0,0", f"{side - 1},{side - 1}"
    rows = []
    for name, solver in [("dijkstra", dijkstra), ("astar", astar)]:
        ...
        for _ in range(repetitions):
            ...
            samples.append(
                {"runtime_ms": elapsed, "visited_nodes": result.visited, "path_cost": result.cost}
            )
        rows.append(
            {
                "algorithm": name,
                ...
                "nodes": side * side,
                "edges": 2 * side * (side - 1),
                "runtime_ms_mean": mean(x["runtime_ms"] for x in samples),
                "visited_nodes_mean": mean(x["visited_nodes"] for x in samples),
                "path_cost": samples[0]["path_cost"],
            }
        )
    return rows
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sizes", nargs="+", type=int, default=[10, 25, 50, 75])
    parser.add_argument("--repetitions", type=int, default=3)
    parser.add_argument("--output", default="data/processed/graph_benchmark.csv")
    args = parser.parse_args()
    for row in run(args.sizes, args.repetitions, args.output):
        print(row)
