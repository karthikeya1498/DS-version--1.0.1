...
The browser operations dashboard is the TypeScript/HTML/CSS application under `frontend/`; it calls the FastAPI application boundary and presents seeded scenario metrics and decision traces. The Streamlit research dashboard remains `dashboard.py` and visualizes the road network, benchmark runtime, and XGBoost forecast metrics. Docker Compose exposes Streamlit at `http://localhost:8501` alongside the API at `http://localhost:8000`.
