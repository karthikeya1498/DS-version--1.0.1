...
def record(payload: DecisionCreate):
    decision = DecisionRecord(decision_id=payload.decision_id, scenario_id=payload.scenario_id, state_reference='', dataset_version=payload.dataset_version, model_versions={}, solver_version=payload.solver_version, rl_policy_version='', selected_action=payload.selected_action, objective_metrics=payload.objective_metrics, candidates=tuple(payload.candidates))
    store.save(decision)
...
def get_decision(decision_id: str):
    ...
    if decision is None: raise HTTPException(status_code=404, detail='decision not found')
...
def compare(payload: dict):
    decision = store.get(payload.get('decision_id', ''))
    if decision is None: raise HTTPException(status_code=404, detail='decision not found')
    return {'decision_id': decision.decision_id, 'candidates': [asdict(candidate) for candidate in decision.candidates], 'selected_action': decision.selected_action}
