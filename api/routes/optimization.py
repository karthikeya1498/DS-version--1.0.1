...
def optimization_demo():
    now = datetime.now(timezone.utc); graph = RoadGraph()
    for n in ['a','b','c']: graph.add_node(Node(n, 0, ord(n)-97))
    graph.add_edge(Edge('a','b',1), bidirectional=True); graph.add_edge(Edge('b','c',1), bidirectional=True)
    origin, destination = Location('a','zone-a',0,0), Location('c','zone-c',0,2)
    order = Order('demo-order', origin, destination, 1, now, TimeWindow(now, now + timedelta(hours=2)))
