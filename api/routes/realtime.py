...
async def traffic_socket(websocket: WebSocket):
    ...
    try: principal = decode_token(token)
    except Exception:
        await websocket.close(code=1008); return
    await websocket.accept(); queue = await traffic_stream.subscribe()
    try:
        ...
        while True:
            try: event = await asyncio.wait_for(queue.get(), timeout=25)
                ...
                await websocket.send_json({'event_type':'heartbeat'}); continue
            await websocket.send_json(event)
    except (WebSocketDisconnect, RuntimeError):
        pass
    finally: traffic_stream.unsubscribe(queue)
